...
def shapesDynamicStyling(elem: HTMLDivElement)(using ge: GeoEnv): Unit =
  val h = document.getElementById("maincanvas").asInstanceOf[HTMLCanvasElement].clientHeight
  elem.style.top = s"${h + 60}px"
  elem.style.height = s"${window.innerHeight - h - 105}px"
...
def renderCurveOnButton(cnvs: HTMLCanvasElement, curveName: String) =
  ...
  cnvs.getContext("2d").clearRect(0, 0, cvsS.width, cvsS.height)
